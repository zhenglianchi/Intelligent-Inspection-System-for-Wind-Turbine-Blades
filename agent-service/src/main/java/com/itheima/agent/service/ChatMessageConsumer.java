...
import com.itheima.agent.dto.ChatMessage;
...
public class ChatMessageConsumer {
    ...
    public void handleChatRequest(ChatMessage message) {
        ...
        try {
            ...
            // 智能历史上下文召回：根据当前问题从完整历史中检索相关对话
            String originalMessage = message.getMessage();
            String enrichedMessage = originalMessage;
            String relevantHistory = historyRetrievalService.retrieveRelevantHistory(message.getMemoryId(), originalMessage);
            if (!relevantHistory.isEmpty()) {
                // 将召回的历史上下文加到用户问题前面
                enrichedMessage = relevantHistory + "\n用户当前问题：" + originalMessage;
                log.info("🧠 [历史召回] 已注入相关历史上下文到用户问题");
            }

            String answer = windFarmAssistant.chat(message.getMemoryId(), enrichedMessage);
